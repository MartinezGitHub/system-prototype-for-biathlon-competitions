...
func (i *implementation) HandleOnStartLine(rest string) (string, error) {
	tokens := lexer.AllTokens(rest)
	eventTime, _, competitorID, err := parseBaseEvent(tokens)
	if err != nil {
		return "", fmt.Errorf("on start line: %w", err)
	}

	event := entity.Event{
		Time:         eventTime,
		Type:         entity.EventOnStartLine,
		CompetitorID: competitorID,
	}
	return fmt.Sprintf("[%s] The competitor(%d) is on the start line",
		event.Time.Format("15:04:05.000"),
		event.CompetitorID), i.competitorsUseCase.ProcessOnStartLine(event)
}
